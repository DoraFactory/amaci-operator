...
const formats = {
  ...
  console: winston.format.combine(
    ...
    winston.format.printf(
      ({ timestamp, level, message, module, round, operation, ...rest }) => {
        const moduleStr = module ? `[${colors.magenta(String(module))}]` : ''
        const roundStr = round
          ? `[${colors.green(`Round:${String(round)}`)}]`
          : ''
...
const cleanupTransports = (forceCleanAll = false) => {
  ...
  if (transportCount > 0 || loggersCount > 0) {
    console.log(
      `[${new Date().toISOString()}][LOGGER] Running cleanup, force=${forceCleanAll}, transports=${transportCount}, loggers=${loggersCount}`,
    )
  ...
  if (forceCleanAll) {
    console.log(
      `[${new Date().toISOString()}][LOGGER] Force cleaning all resources (${transportCount} transports, ${loggersCount} loggers)`,
    )
    ...
    for (const [roundId, logger] of roundLoggers.entries()) {
      try {
        ...
        console.error(
          `[${new Date().toISOString()}][LOGGER] Error during force cleanup of logger ${roundId}: ${e}`,
        )
      }
    }
    ...
    for (const [roundId, transport] of roundTransports.entries()) {
      try {
        ...
        console.error(
          `[${new Date().toISOString()}][LOGGER] Error during force cleanup of transport ${roundId}: ${e}`,
        )
      }
    }
  ...
  if (cleanedCount > 0) {
    console.log(
      `[${new Date().toISOString()}][LOGGER] Cleaned up ${cleanedCount} inactive resources`,
    )
  }
}
    ...
    if (roundLoggers.has(roundId)) {
      try {
        ...
        if (e instanceof Error && !e.message.includes('not found')) {
          console.error(
            `[${new Date().toISOString()}][LOGGER] Error closing logger for ${roundId}: ${e}`,
          )
        }
      }
    }
...
export const closeAllTransports = async () => {
  ...
  if (isClosingLoggers) {
    console.log(
      `[${new Date().toISOString()}][LOGGER] Already closing loggers, skipping duplicate call`,
    )
    return false
  }
...
export const emergencyCloseLoggers = () => {
  ...
  try {
    ...
    console.log(
      `[${new Date().toISOString()}][LOGGER] Emergency shutdown completed`,
    )
  } catch (e) {
    // ignore errors, ensure not blocking process exit
    console.error(
      `[${new Date().toISOString()}][LOGGER] Error during emergency shutdown:`,
      e,
    )
  }
}
    ...
    try {
      ...
      if (forceRebuild) {
        ...
        if (rebuildCount % 10 === 1) {
          console.log(
            `[${new Date().toISOString()}][LOGGER] Rebuilding logger for round ${roundId} (rebuild #${rebuildCount}, after ${writeCount} writes)`,
          )
        }
      }
      ...
      console.error(
        `[${new Date().toISOString()}][LOGGER] Error writing to round logger: ${e}`,
      )
...
export const endOperation = (
  operationName: string,
  success: boolean,
  operationContext: Record<string, any> | string = {},
) => {
  ...
  if (
    finalContext.operations &&
    finalContext.operations[operationName]?.startTime
  ) {
    ...
    // other cases, try to force conversion
      ...
      try {
        ...
        console.error(
          `Failed to calculate duration for operation ${operationName}: Invalid startTime format`,
        )
      }
    }
  }
  ...
  if (duration < 1) {
    duration = 1
    console.warn(
      `Operation ${operationName} had zero duration, setting to 1ms minimum`,
    )
  }
...
export const separator = (title?: string) => {
  ...
  if (title) {
    ...
    line =
      ...
      title +
      ' ' +
      '='.repeat(lineWidth - padding - title.length - 2)
  } else {
...
export const gracefulShutdown = async (timeout = 5000): Promise<boolean> => {
  ...
  return new Promise((resolve) => {
    ...
    const timeoutHandle = setTimeout(() => {
      console.warn(
        `[${new Date().toISOString()}][LOGGER] Shutdown timeout after ${timeout}ms - forcing exit`,
      )
      emergencyCloseLoggers()
      resolve(false)
    }, timeout)
    ...
    for (const [roundId, logger] of roundLoggers.entries()) {
      closePromises.push(
        new Promise<void>((resolveLogger) => {
          ...
          console.error(
            `[${new Date().toISOString()}][LOGGER] Error closing logger for round ${roundId}:`,
            err,
          )
        }),
      )
    ...
    Promise.all(closePromises)
      .then(() => {
        ...
        console.log(
          `[${new Date().toISOString()}][LOGGER] All loggers closed successfully`,
        )
        resolve(true)
      })
      .catch((err) => {
        clearTimeout(timeoutHandle)
        console.error(
          `[${new Date().toISOString()}][LOGGER] Error during graceful shutdown:`,
          err,
        )
        emergencyCloseLoggers()
        resolve(false)
