...
export const tally: TaskAct = async (_, { id }: { id: string }) => {
  // 开始记录 tally 操作时间
  const startTime = startOperation(id, 'tally');
  ...
  try {
    ...
    if (
      ...
      // 记录失败的操作
      endOperation(id, 'tally', false, startTime, 'error status: not end');
    ...
    // 记录成功的操作
    endOperation(id, 'tally', true, startTime, undefined);
    return {}
  } catch (error: any) {
    // 记录失败的操作
    endOperation(id, 'tally', false, startTime, error.message);
    throw error;
  }
}
