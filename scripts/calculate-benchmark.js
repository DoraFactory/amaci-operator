...
const COINGECKO_API = 'https://api.coingecko.com/api/v3/simple/price?ids=dora-factory-2&vs_currencies=usd&include_24hr_change=true';
// Fetch DORA price from CoinGecko
async function fetchDoraPrice(retries = 3) {
...
function loadCachedPrice() {
    try {
        if (fs.existsSync(REPORT_JSON_PATH)) {
            const previousReport = JSON.parse(fs.readFileSync(REPORT_JSON_PATH, 'utf-8'));
            if (previousReport.doraPrice && previousReport.doraPrice > 0) {
                console.log(`⚠ Using cached DORA price: $${previousReport.doraPrice.toFixed(4)}`);
                return {
...
// Calculate costs
function calculateCosts(config, doraPrice) {
    const result = {};
    for (const [machineType, machineConfig] of Object.entries(config.machineTypes)) {
        const costs = {};
        for (const [scale, baseUsd] of Object.entries(machineConfig.currentTallyCostUsd)) {
...
function generateJsonReport(doraPrice, dora24hrChange, machineTypes, previousReport) {
    const report = {
...
function generateMarkdownReport(report) {
    ...
    lines.push('');
    lines.push(`**Generated:** ${new Date(report.timestamp).toLocaleString('en-US', { timeZone: 'Asia/Shanghai', hour12: false })} (Beijing Time)`);
    lines.push('');
    // DORA Price section
    ...
    if (report.dora24hrChange !== undefined) {
        const changeStr = report.dora24hrChange > 0
            ? `+${report.dora24hrChange.toFixed(2)}%`
            : `${report.dora24hrChange.toFixed(2)}%`;
        const emoji = report.dora24hrChange > 0 ? '📈' : report.dora24hrChange < 0 ? '📉' : '➡️';
        lines.push(`**24h Change:** ${changeStr} ${emoji}`);
    }
    if (report.previousReport) {
        const priceChange = ((report.doraPrice - report.previousReport.doraPrice) /
            report.previousReport.doraPrice) *
            100;
        lines.push('');
        lines.push('**Comparison with Previous Report:**');
        lines.push(`- Previous Price: $${report.previousReport.doraPrice.toFixed(4)}`);
        lines.push(`- Previous Time: ${new Date(report.previousReport.timestamp).toLocaleString('en-US', { timeZone: 'Asia/Shanghai', hour12: false })}`);
        lines.push(`- Price Change: ${priceChange > 0 ? '+' : ''}${priceChange.toFixed(2)}%`);
    }
    lines.push('');
    // Cost tables for each machine type
    for (const [machineType, machineData] of Object.entries(report.machineTypes)) {
        lines.push(`## ${machineType}`);
        lines.push('');
        lines.push('| Scale | Base Cost (USD) | Multiplier | Final Cost (USD) | DORA Amount | Change |');
        lines.push('|-------|----------------|------------|------------------|-------------|--------|');
        for (const [scale, cost] of Object.entries(machineData.costs)) {
            ...
            lines.push(`| ${scale} | $${cost.baseUsd.toFixed(2)} | ${cost.multiplier}x | $${cost.finalUsd.toFixed(2)} | ${cost.doraAmount.toFixed(2)} DORA |${changeIndicator} |`);
        }
        lines.push('');
    }
    // Calculation formula
...
async function main() {
    try {
        ...
        if (previousReport) {
            console.log(`✓ Previous report loaded (from ${previousReport.timestamp})\n`);
        }
        else {
        ...
        const jsonReport = generateJsonReport(doraPrice, change24h, machineTypes, previousReport);
        ...
        console.log(`   Total Scales: ${Object.values(machineTypes).reduce((sum, m) => sum + Object.keys(m.costs).length, 0)}`);
    }
    catch (error) {
        console.error('\n❌ Error:', error);
        process.exit(1);
